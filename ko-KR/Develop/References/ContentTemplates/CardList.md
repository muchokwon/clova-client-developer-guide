...
| `noticeText`                | [StringObject](/Develop/References/ContentTemplates/Shared_Objects.md#StringObject)       | 콘텐츠의 저작권이나 법적인 사항과 관련된 내용이 담긴 객체. 이 객체의 `value` 필드는 빈 문자열(`""`)을 가질 수도 있습니다. **이 객체 필드의 값이 빈 문자열(`""`)이 아니면 반드시 화면에 표시해야 합니다.**  |
