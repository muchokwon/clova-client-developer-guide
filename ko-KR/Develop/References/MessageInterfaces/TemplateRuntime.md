...
| [`MoreCommandIssued`](#MoreCommandIssued)               | Event     | 사용자가 클라이언트 기기의 미디어 플레이어에서 특정 미디어에 대해 더보기 버튼(More)을 눌렀을 때 클라이언트는 이 이벤트 메시지를 CIC에게 전송해야 합니다. |
...
사용자가 클라이언트 기기의 미디어 플레이어에서 특정 미디어에 대해 좋아요 버튼(Like)을 눌렀을 때 클라이언트는 이 이벤트 메시지를 CIC에게 전송해야 합니다. 이 이벤트 메시지를 받은 CIC는 상황에 맞는 지시 메시지를 클라이언트에게 전송합니다.
...
사용자가 클라이언트 기기의 미디어 플레이어에서 특정 미디어에 대해 더보기 버튼(More)을 눌렀을 때 클라이언트는 이 이벤트 메시지를 CIC에게 전송해야 합니다. 이 이벤트 메시지를 받은 CIC는 상황에 맞는 지시 메시지를 클라이언트에게 전송합니다.
