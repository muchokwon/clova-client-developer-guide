...
| `initiator`                                              | object   | Contains the information on the Clova call method used, voice input source, and wake word.<div class="tip"><p><strong>Tip!</strong></p><p>The use of this field will improve the performance of voice recognition. Therefore, we recommend that you use this field.</p></div>                      | Optional    |
...
| `initiator.type`                                         | string   | The invoke method used. Available values are: <ul><li><code>"PRESS_AND_HOLD"</code>: Voice inputted while pressing the wake up button</li><li><code>"TAP"</code>: Voice inputted after pressing and releasing the wake up button</li><li><code>"WAKEWORD"</code>: Voice inputted after saying the wake word</li></ul>  | Required |
