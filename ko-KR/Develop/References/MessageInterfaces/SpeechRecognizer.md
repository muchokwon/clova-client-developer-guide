...
| `initiator.payload.wakeWord.indices.endIndexInSamples`    | number   | 오디오 스트림에서 호출어가 끝나는 시점의 index 정보. 음성 입력이 16 kHz sample rate를 가지므로 index의 1 단위는 16,000 분의 1 초를 의미합니다. 만약, 오디오 스트림에서 ​호출어 구간이 1 초 위치에서 끝난다면 이 필드의 값으로 1 초를 의미하는 `16000`을 입력해야 합니다.  | 필수  |
| `initiator.payload.wakeWord.indices.startIndexInSamples`  | number   | 오디오 스트림에서 호출어가 시작되는 시점의 index 정보. 음성 입력이 16 kHz sample rate를 가지므로 index의 1 단위는 16,000 분의 1 초를 의미합니다. 호출어는 대체로 사용자 발화의 첫 부분에 위치하기 때문에 index 값을 0으로 입력하게 됩니다.   | 필수 |
